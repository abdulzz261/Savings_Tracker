    ...
    // VARIABLE KONTROL ANIMASI DELAY
    let pendingPercentage = 0;
    let isGoalCompletedCelebration = false;
    let isEstimasiAnimating = false;
    let editModalInterval = null;

    function verifyGoalData() {
      const goalName = localStorage.getItem('rdpu_goal_name');
      if (!goalName) { window.location.replace('index.html'); return false; }
      return true;
    }

    window.addEventListener('pageshow', function(event) {
      if (event.persisted || !verifyGoalData()) { window.location.replace('index.html'); }
    });
  ...
  const btnEl = document.getElementById('btn-toggle-est');
  ...
  if (box.classList.contains('active')) {
    ...
    // UBAH STATE TOMBOL JADI DEFAUL/SEMBUNYI
    ...
    setTimeout(() => { 
      ...
      isEstimasiAnimating = false; 
    }, 250);
  } else {
    box.classList.remove('hiding');
    box.classList.add('active');
    
    // UBAH STATE TOMBOL JADI AKTIF (HIJAU PASTEL)
    ...
    function showModal(title, desc, type, isConfirmReset = false, onConfirm = null) {
      ...
      if (isConfirmReset) {
        actionsEl.innerHTML = `
          <button class="btn-modal-cancel" onclick="closeModal(false)">Batal</button>
          <button class="btn-modal-confirm" id="confirmModalBtn">Ya, Lanjutkan</button>
        `;
        document.getElementById('confirmModalBtn').onclick = () => { 
          closeModal(true); // Tutup modal konfirmasi tanpa animasi bar dulu
          if (onConfirm) onConfirm(); 
        };
      } else {
        actionsEl.innerHTML = `<button class="btn-modal-primary" onclick="closeModal(true)">OK, Siap!</button>`;
      }
      modalEl.classList.add('active');
      document.body.classList.add('modal-open');
    }

    // PARAMETER shouldAnimate: HANYA ANIMASIKAN BAR JIKA NOTIF BENERAN DITUTUP PERMANEN
    function closeModal(shouldAnimate = true) { 
      ...
      if (shouldAnimate) {
        applyPendingUIUpdate();
      }

      if (isGoalCompletedCelebration) { triggerLongCelebration(); isGoalCompletedCelebration = false; }
    }

    function applyPendingUIUpdate() {
      document.getElementById('percent-text').innerText = pendingPercentage.toFixed(1) + '%';
      
      const fillEl = document.getElementById('progress-fill');
      if (fillEl) fillEl.style.width = pendingPercentage.toFixed(1) + '%';
    }
    ...
    function renderUI(currentAmount) {
      ...
      // HANYA SIMPAN KE VARIABLE PENDING, JANGAN UBAH ELEMEN DOM SAMA SEKALI DI SINI!
    ...
    window.onload = function() {
      ...
      window.onpopstate = function () {
        if (!localStorage.getItem('rdpu_goal_name')) { window.location.replace('index.html'); }
        else { history.pushState(null, null, location.href); }
      };

      recalculateAllHistoryAndSaldo();
      // PERTAMA KALI LOAD HALAMAN: LANGSUNG JALANKAN ANIMASI AWAL
      applyPendingUIUpdate();
    };